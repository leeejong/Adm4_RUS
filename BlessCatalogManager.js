...
define([/*Require*/], function(){
   ...
   * @param {mixed} arg2
   */
  return function BlessCatalogManager( arg1, arg2 ){
    ...
    var info_guide = {
     1 : '<br>▶ 판매 또는 지급용 상품을 만들 수 있습니다.<br><br> <b>- 판매용 상품</b> : 빌링시스템에 상품정보(SALEINFO_ID)를 등록한 후 웹 상점에서 판매를 목적으로 만드는 상품. 판매 가격이 필수<br> \n\
<b>- 지급용 상품</b> : 패키지 아이템의 구성품으로 포함되어 지급되는 상품이며, 단품으로 판매불가<br><b>- 상품판매번호</b> : 빌링시스템(NBOS)의 SALEINFO_ID를 지칭함',
     2 : '<br>▶ 판매 또는 지급용 상품 목록을 조회하고 일부 정보를 수정할 수 있습니다.<br><br> <b> 주의!! 한번 등록된 상품의 지급 아이템 구성은 변경이 불가능 합니다. 만약 판매 전일 경우에는 관리자에게 요청해주세요.</b><br><br>\n\
<b>- 상품판매번호(SALEINFO_ID)발급</b> : 판매 목적으로 만든 상품의 결제정보를 빌링시스템(NBOS)에 등록하는 과정입니다. 상품 가격이 설정되어 있어야 하며(발급가능), 등록된 정보로 빌링 시스템에 상품 정보를 자동 등록 합니다. 상품정보가 등록된 이후 상품판매번호는 게임DB에 업데이트 됩니다.',
     3 : '<br>▶ 웹 상점의 카테고리 메뉴를 구성하고, 판매를 위한 상품을 원하는 곳에 진열할 수 있습니다.<br><br><b>- 카테고리 메뉴</b> : 웹상점의 판매 상품을 특성 및 목적에 따라 분류하는 기능을 담당 합니다. 카테고리 메뉴의 추가/수정/삭제는 마우스 우클릭을 통해 실행 가능 합니다. 카테고리 메뉴는 최대 2단계 까지 구성이 가능 합니다.<br>\n\
<b>- 카테고리 등록 상품</b> : 선택한 카테고리에 등록된 판매상품 정보를 조회합니다. 노출 순서가 높을 수록 상위에 노출 됩니다. [삭제] 버튼을 통해 상품을 내릴 수 있습니다.<br>\n\
<b>- 전체 판매 상품</b> : 판매를 목적으로 등록한 상품 중 상품판매번호가 있는 상품 정보가 표시 됩니다. [상품 올리기] 버튼을 통해 상품을 진열할 수 있습니다.<br>\n\
<b>- 할인 카테고리</b> : 카테고리에 진열된 상품은 할인 이벤트와 연계되며 이벤트가 종료되면 해당 상품의 판매가 종료 됩니다.',
     4 : '<br>▶ 할인 이벤트(가격할인, 한정판매)를 관리 할 수 있습니다.<br><br> <b>- 이벤트 정보</b> : 이벤트 정보의 상세정보 조회 및 수정을 할 수 있습니다.<br>\n\
<b>- 할인 상품 관리</b> : 선택한 이벤트 회차의 할인 상품을 등록할 수 있습니다.<br><br>\n\
<b>- 할인 상품 등록 방법</b><br>1. 이벤트 정보를 새로 등록 합니다. 이벤트 기간 및 보너스 상품 정보를 설정할 수 있습니다.<br>2. [할인 상품 관리] 메뉴에서 할인 이벤트 회차를 선택 합니다.<br>3. 하단의 카테고리 메뉴에서 할인을 적용할 상품을 선택 합니다.\n\
<br>4. 상품 정보를 조회한 후 [할인추가] 버튼을 클릭 합니다.<br><br>\n\
<b>- 한정 판매 상품 </b>: 한정 판매수량이 0보다 큰 값인 경우, 모든 수량이 팔리면 해당 이벤트 회차에서는 더이상 판매가 불가능 합니다.<br>\n\
<b>- 가격 할인 상품 </b>: 한정 판매수량이 0으로 설정된 경우, 수량제한 없이 해당 이벤트 회차 동안 할인이 적용 됩니다.'
    }
    ...
    */
    ...
    var b_menu_1  = $E('button').text('1. 상품 만들기').css({'width':210, 'text-align':'left'}).button().click(function(){  goMakeCashItem() }),
        b_menu_2  = $E('button').text('2. 상품 및 빌링(NBOS)정보 관리').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageCashItem() }),
        b_menu_3  = $E('button').text('3. 판매상품 진열하기').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageCategory()  }),
        b_menu_4  = $E('button').text('4. 할인 이벤트 관리').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageDscInfo()  }),
        b_menu_5  = $E('button').text('7. 동기화 실행').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageSync()  }),
        b_menu_6  = $E('button').text('5. 블레서쉽 등급 관리').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageGrade()  }),
        b_menu_7  = $E('button').text('6. VIP혜택 관리').css({'width':210, 'text-align':'left'}).button().click(function(){ goManageVip()  });
    ...
    //사용법 가이드
    div_guide.verticalTable([[ $E('lable').html(info_guide[1]), $E('lable').html(info_guide[2]), $E('lable').html(info_guide[3]), $E('lable').html(info_guide[4]) ]], {head_width : 240, head: ['1. 상품 만들기', '2. 상품 및 빌링(NBOS)정보 관리', '3. 판매상품 진열하기', '4. 할인 이벤트 관리'] , 'width':1300});
    ...
    function goManageGrade()
    {
      ...
      div_guide.empty();
      var sub_seletwin  = _drawWin( '블레서쉽 등급 관리',sub_contents, 0, 10, 1310, 650);
            ...
            sub_selectinfo.empty().clientTable(itemreg_list, {
              ...
              translate: {
                ...
                dist_price: function(v, cname, row){
                  return $E('lable').text( set_comma(v) + '원').css({'color':'red'});
                 },
              ...
              add_column: {
                  'detail': function(row){
                    var b_manage = $E('button').text('정보').css({width:80,height:35}).button({icons:{primary:'ui-icon-zoomin'}}).click(function(){

                      var div_info = $E('div'),
                          div_bottom = $E('div').css({'text-align':'center'});

                      var l_dsc_id      = $E('lable').text(row.dsc_id + '회차'),
                          ...
                          head: {
                           ...
                           translate_realtime: {
                            coupon_id : function(v, name, r ){
                             ...
                             price: function(v){
                              return $E('lable').text( set_comma(v) + '원').css({'color':'black', 'text-align':'right'});
                             },
                             dist_price: function(v, name, row){
                              return $E('lable').text( set_comma(v) + '원').css({'color':'red', 'text-align':'right'});
                             }
                           },
                          'footer': false
                          }
                  ...
                  var b_manage = $E('button').text('상세정보').button({icons:{primary:'ui-icon-zoomin'}}).css({width:90,height:35}).click(function(){
                    try {
                      ...
                      var l_cash_item_id        = $E('lable').text(row.cash_item_id),
                          ...
                          l_price               = $E('lable').text(set_comma(row.price)+'원'),
...
*/
        ...
        sub_selectitem.empty().clientTable( list, {
              ...
              translate: {
                ...
                saleinfo_id : function(value, name, row) {
                   ...
                   else if (row.price > 0) _com.val('발급가능').css({'color':'#ee4411'});
                   else _com.val('발급불가').css({'color':'black'});;
                   return _com;
                 },
              ...
              add_column: {
                  'detail' : function(row){
                    var b_manage = $E('button').text('정보').button({icons:{primary:'ui-icon-zoomin'}}).css({width:70,height:30}).width('100px').click(function(){
                      try {
                        showCashItemDetail(row.cash_item_id);
                      }catch(e) {alert("[오류!] 상품 정보를 갖고올 수 없습니다.")};
                    });
                    var b_delete = $E('button').text('삭제').button({icons:{primary:'ui-icon-trash'}}).css({width:70,height:30}).width('100px').click(function(){
                      try {
                        if (!confirm("선택한 상품을 삭제하시겠습니까?\n아래의 정보가 모두 함께 삭제 됩니다.\n(상품정보, 판매진열정보, 할인정보, 패키징정보)")) return;
                        if (delCashItem(row.cash_item_id))
                          getCashItemList();
                      }catch(e) {alert("[오류!] 상품 정보를 삭제할 수 없습니다.")};
                    });
      ...
      var  div_info        = $E('div').css({'align':'center'}),
           ...
           b_sel_all       = $E('button').text('전체 보기').button({icons:{primary:'ui-icon-search'}}).css({height:50}).click(function(){
           ...
           b_sel_able       = $E('button').text('판매 예정상품 보기').button({icons:{primary:'ui-icon-search'}}).css({height:50}).click(function(){
           ...
           b_sel_reg       = $E('button').text('상품판매번호 (SALEINFO_ID) 발급').button().css({"color":"white", "height":50, "background":"#ee4411"}).click(function(){ goRegAll() }).appendTo(div_bottom);
    ...
    function goMakeCashItem()
    {
      ...
      var sub_selectnew   = _drawWin( '신규 상품 정보', sub_contents, 0, 10, 560, 680);
      var sub_selectitem  = _drawWin( '지급 아이템/상품 정보',sub_contents, 0, 575, 745, 680);

      var div_info    = $E('div'),
          div_bottom  = $E('div').css({'text-align':'center'}).append('<br>');;

      var l_cash_item_id        = $E('lable').text('신규생성'),
          ...
          }),  $E('lable').text('신규생성 '), '<nbsp;>', $E('radio', {name:'sel_item_code'}).val('manual').attr("checked", false).click(function(){

            t_cash_item_code.attr('readonly',false);

          }) , $E('lable').text('수동입력 : '), t_cash_item_code ),
          ...
          }),  $E('lable').text('게임아이템 '), '<nbsp;>', $E('radio', {name:'sel_sub_item_type'}).val('cash').attr("checked", false).click(function(){
          ...
          t_ref_item_id_reset   = $E('button').text('모두삭제').button({icons:{primary:'ui-icon-trash'}}).css({height:25}).click(function(){

            t_ref_item_id.html('');

          }),

          b_add         = $E('button').text('상품정보 등록').button({icons:{primary:'ui-icon-check'}}).css({height:50}).appendTo(div_bottom).click(function(){
            ...
            if ( $('input[name=sel_item_code]:checked').val() == 'auto' )
              ...
              if (t_cash_item_code.val() == '') return alert('상품 코드를 입력해 주세요.');
            }

            if (t_name.val() == '') return alert('상품명을 입력해 주세요.');
            if (s_item_type.val() == 'VP' && (t_period.val() == "" || isNaN(t_period.val()) == true )) return alert('VIP 효력(일)/개수 항목은 숫자만 입력해 주세요.');
            if (t_price.val() == "" || isNaN(t_price.val()) == true ) return alert('판매가격은 숫자만 입력해 주세요.');
            if (s_item_type.val() != 'LM' && s_item_type.val() != 'VP' && t_ref_item_id.html() == '') return alert('지급 아이템을 최소 1개이상 등록해 주세요.');
            if (s_item_type.val() == 'LM' && (t_reward_money.val() <= 0 || isNaN(t_reward_money.val()) == true) ) return alert('지급 루메나를 입력해 주세요.');
            ...
            menu.ajax({
              ...
              success    : function( result ){
                alert('상품정보가 성공적으로 등록 되었습니다.');
              }
            });
          });

          sub_selectnew.verticalTable([[ l_cash_item_id, l_cash_item_code, t_name, s_item_type, l_sub_item_type,  s_vip_level, s_period_type, t_period, t_price, t_saleinfo_id, t_description, t_short_description,  s_sale_place, s_sale_yn, $E('div').append(t_ref_item_id).append(t_ref_item_id_reset), t_reward_money, $E('div').append(c_dup_yn, $E('lable').text('중복구매 불가 상품'))  ]], {head: ['상품 번호(ID)', '상품 코드', '상품명', '상품유형', '*지급 아이템/상품 정보', '*패키지 등급', '*효력 구분', '*효력(일,월,년) 개수', '판매 가격', '상품판매번호', '상세 설명(PC)', '짧은 설명', '판매 위치', '판매 유무', '지급 아이템', '지급 루메나', '구매옵션']}).append(div_bottom);
          ...
          function getGameBoosterList()
          {
            ...
            sub_selectitem.empty().clientTable( list, {
                  ...
                  add_column: {
                      'sel' : function(row){
                        var b_manage = $E('button').text('추가').button({icons:{primary:'ui-icon-plus'}}).width('100px').click(function(){
                          try {
          ...
          function getGameItemList()
          {
            ...
            sub_selectitem.empty().clientTable( gameitem_list, {
                  ...
                  add_column: {
                      'desc': function(row) { return $E('lable').text('게임아이템') },
                      'sel' : function(row){
                        var b_manage = $E('button').text('추가').button({icons:{primary:'ui-icon-plus'}}).width('100px').click(function(){
                          try {
