...
(function ($, window, document) {
   ...
   */
    ...
    return function BlessServer(arg1, arg2) {
      ...
      var menu = this;
      var desc = {
        open_login : {0:'서버 닫힘'.t('BLESS'), 1:'서버 열림'.t('BLESS')},
        stop_all_trade : {0:'정지 아님'.t('BLESS'),1:'정지'.t('BLESS')},
        stop_auction : {0:'정지 아님'.t('BLESS'),1:'정지'.t('BLESS')},
        stop_mail : {0:'정지 아님'.t('BLESS'),1:'정지'.t('BLESS')},
        stop_personal_trade : {0:'정지 아님'.t('BLESS'),1:'정지'.t('BLESS')},
        stop_warehouse : {0:'정지 아님'.t('BLESS'),1:'정지'.t('BLESS')},
        restrict_realm_hieron : {0:'Normal',1:'LimitRealm',2:'System'},
        restrict_realm_union : {0:'Normal',1:'LimitRealm',2:'System'}
      }
...
//      });
        ...
        cuWin.ajax({
          ...
          success: function (jorbiCU) {
            cuWin.clientTable(jorbiCU[0],{
              title:'전서버 실시간 정보'.t('BLESS'),
              width:'100%',
              field: ['totalcu','change','waiting','lobby','ingame','hiran','union','hcount','ucount','usncount','regdate'],
              head : ['TotalCU','Change','Waiting','Lobby','Ingame','H_CU','U_CU','H_USN','U_USN','TotalUSN','Regdate'],
              translate_realtime : {
                change : function(v, cname, row){
                  ...
                  if(Math.abs(row.change) > wholeDeltaSetting && wholeDeltaSetting!=-1){
                    ...
                    alert('전서버 - 접속자 변화를 감지했습니다.'.t('BLESS'));
                  ...
                  if(row.totalcu >= wholeMaxSetting  && wholeMaxSetting!=-1){
                    alert('전서버 - 최대접속자 기준에 도달했습니다'.t('BLESS'));
                  }else if(row.totalcu < wholeMinSetting  && wholeMinSetting!=-1){
                    alert('전서버 - 최소접속자 기준을 넘었습니다'.t('BLESS'));
                  }
      ...
      get(); setAlert(); //drawGraph();
      var intv = setInterval( function(){ get(); /*drawGraph();*/ } , 1000*60*1 );
